...
Boneclang::Boneclang(const GameObject* const pTarget, const Point2f& bottomLeft) :
	...
	HitBox(m_RootCenter),
	SimpleSprite(m_RootCenter, 0.0f, 4.0f, "Resources/Entities/Boneclang.png", 8),
...
void Boneclang::OnHurt(const Vector2f& hurtNormals, int damage, int localHurtBoxIndex)
{
	...
	if (!--m_Health)
	{
		...
		GameObject::Delete();
		GameObject::AddGameObject(new FX{ m_RootCenter.x, m_RootCenter.y - 8.0f, "Resources/FX/Poof.png", 5, 8, 0.1f });
		GameObject::AddGameObject(new FX{ m_RootCenter.x, m_RootCenter.y + 8.0f, "Resources/FX/Poof.png", 5, 8, 0.1f });
...
void Boneclang::OnColumnChange(int currentColumnIndex)
{
	if (m_State == State::Attack)
	{
		...
		case 1:
			SoundManager::PlayEffect("Resources/Audio/BoneclangAttack.wav", false);
